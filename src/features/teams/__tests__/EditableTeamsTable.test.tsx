...
describe("EditableTeamsTable Component", () => {
  ...
  test("should show unknown fallback for member without name", () => {
    const teamsWithNullMember: CombinedTeam[] = [
      {
        ...mockCombinedTeams[0],
        // Deliberately-invalid runtime value: name is `null` to exercise the
        // "unknown" fallback even though the type declares it as `string`.
        members: [
          {
            personId: "person-x",
            name: null,
            email: "x@example.com",
          } as unknown as CombinedTeam["members"][number],
        ],
      },
    ];
